...
export class PartView {
  ...
  loadPArtOrderLines(){
    var ets = (this.filter.ets!=null)?this.filter.ets.map(function(item){return "&rs:ets=" + item.id }).join(""):""
    ...
    , {
      ...
      .then(data => {
        ...
        this.loadFournisseurNames();
        this.getCurrentETSNames();
      })
  };


  loadFournisseurNames(){
    let references = this.catalogEntries.map(function(item){return item.FOUSBART})
    ...
    this.http.fetch('/v1/resources/fournisseur', {
      headers: {
        'Content-Type': 'application/json'
      },
      method: 'post',
  ...
  getCurrentETSNames(){
    let query = {
      "ets" : this.orderLines.map(function(item){return item.ETSSALCA})
    }
    ...
    this.http.fetch('/v1/resources/etablissement',
    {
      ...
      .then(data => {
        ...
        data.forEach(function(item){this.etablissements[item.id]=item}.bind(this));
      })
  };
  ...
  getTopThreeNames(buyers){
    if(buyers)
    return buyers.map(function(item){return item.EtablissementName}).join(" ; ")
  };
